...
| Sharing | `backend/share_filter.py`, share endpoints in `main.py`, `ShareModal.tsx` | A saved rule set picks who an archive is for. Pure module, no FastAPI or ORM, so it can be run against a DB copy from a script |
...
- **A row that travels between databases needs a `stable_id`.** A local integer id means nothing in somebody else's project, so without one a merge has to *guess* which local row an incoming one is — and for a document, with no name or birth year to score, it cannot guess at all. `STABLE_ID_TABLES` in `database.py` is the one list; the ALTER, the index and both backfills read it. Two rules that are easy to get backwards: the id is filled in on the **source** before the export copy is taken (`ensure_stable_ids`), never in the copy, or the same row leaves under a new identity every send; and a row created *from* an incoming one keeps the incoming id (`_claim_stable_id`), because that is what makes the next exchange recognise it.
...
- **Importing anything from `backend` boots the app against the real `projects/`.** `project_manager.py` instantiates `ProjectManager()` at *module scope*, and its `__init__` activates a project and runs `init_db_schema` on it. So a bare `python -c "from backend import main"` — an import check, a one-off script, anything — silently migrates the user's active database. Every script that touches `backend`, even to check that it imports, must set `MNEMOSYNE_APP_DIR` to a scratch copy **first**; the variable is read at import time, so setting it inside the script after the import is too late. Symptom: the work looks clean and `git status` is unchanged, while a real project's `schema_version` has moved.
...
- **A new modal's panel must call `stopPropagation()` on its own click.** `useBackdropClose` in `modalBackdrop.ts` closes whenever a click's mousedown and click share a target — it never checks that the target *is* the backdrop. A panel that omits it closes on every button inside it, and the symptom is not "the modal is broken": it is the app apparently dumping you back on whatever tab was underneath. Every other modal in the repo already does this; grep for `stopPropagation` in a new one before believing it works.
- **Two features that delete from the same tables need one list of those tables.** Share-profile content strips and living-person redaction answer different questions and do the same work underneath, so `_redact_persons` is built from the same `_strip_*` helpers `apply_content_strips` dispatches to. Keeping them as separate copies is how a table gets added to one and forgotten in the other, and here that is a leak rather than a bug.
- **An empty selection is not the absence of one.** `build_export_db` reads `person_ids=None` as "no person filter" and `person_ids=[]` as "keep nobody". Testing truthiness instead of `is not None` collapses the two and turns a profile that selects nobody into an export of the entire family — the worst outcome this code has. The same trap sits in `api.ts` (`if (personIds?.length)` omits the parameter) and in a FastAPI `Query(default="")`, where absent and empty arrive as the same string.
- **"Which photos travel" and "who is named on them" are different questions.** The first comes from the cluster derivation at the top of `build_export_db`; the second from whether a cluster keeps its `person_id`. A change made in the wrong one is silent: narrowing the second gives the user unlabelled faces rather than a smaller archive, and narrowing the first without keeping the second wide delivers photographs with half the family anonymous.
- **Kinship distance is measured over blood links only.** `Tree.blood` in `share_filter.py` holds parent and child edges; marriages are not steps. Counting a marriage as one hop makes a spouse's entire family rank nearer than one's own cousins, which is not what anybody means by "close relative". Spouses are added afterwards at their partner's degree, without walking on from them.
- **Redaction is not deletion, and the photographs are the part that gets missed.** A redacted person's columns are blanked in `_redact_persons`, but their pictures are governed by the *cluster* derivation much earlier in `build_export_db`. Blanking the row after their cluster has already pulled every photo of them into the archive redacts nothing — which is why `redact_person_ids` is subtracted from the cluster list too. Anything else hanging off a person by a route other than `person_id` has the same shape.
- **The obvious "is this person alive?" test hides most of a genealogy.** "No death year" is not "living": in an old tree most people's deaths were never written down. The birth year has to be checked too (`is_living` in `share_filter.py`). The tree tab's older `excludeLiving` checkbox had this bug and would drop people born in the 1800s from an export meant to be *about* them.
