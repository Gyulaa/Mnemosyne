...
export default function AssistantSetup({ settings, onDone }: { settings: AiSettings; onDone?: () => void }) {
  ...
  return (
    ...
    <div className="flex-1 min-h-0 overflow-y-auto p-5 pb-8 space-y-5">
      ...
      {/* The app is local-first; sending data to a cloud model is a real change
          in posture, so it is stated plainly rather than buried in settings. */}
      <div className="rounded-lg border border-amber-800/40 bg-amber-950/20 px-3 py-2.5">
        <p className="text-[11px] text-amber-200/90 leading-relaxed">{t('chat.setup.privacy')}</p>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-1.5">
          {(settings.providers ?? []).map(p => {
            const hasKey = settings.configured_providers?.[p.id]
            return (
              <button
                key={p.id}
                onClick={() => { setProvider(p.id); setError(null); refresh.reset() }}
              ...
              >
                ...
                {hasKey && (
                  <span
                    ...
                    className={`absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full ${provider === p.id ? 'bg-white/80' : 'bg-emerald-500'}`}
                  />
                )}
              </button>
            )
      ...
      <label className="flex items-start justify-between gap-3 cursor-pointer">
        ...
        <button
          role="switch"
          aria-checked={settings.allow_private}
          onClick={() => togglePrivate.mutate(!settings.allow_private)}
          className={`inline-flex items-center rounded-full transition-colors shrink-0 mt-0.5 ${settings.allow_private ? 'bg-brand-500' : 'bg-zinc-700'}`}
          style={{ width: '32px', height: '18px' }}
        >
          <span
            className="inline-block w-3.5 h-3.5 rounded-full bg-white shadow transition-transform"
            style={{ transform: settings.allow_private ? 'translateX(16px)' : 'translateX(2px)' }}
          />
        </button>
      </label>
