...
def _t_get_ancestors(ctx: ToolContext, a: dict[str, Any]) -> Any:
    ...
    Genealogical trees repeat given names relentlessly — this project has four
    people called "Miklós Samu", two of them father and son. A model asked to
    trace a line by chaining lookups will sooner or later collapse two of them
    into one and silently drop a generation. So the walk happens here, and the
    answer carries ids and generation numbers the model cannot mix up.
    """
    pid = int(a["person_id"])
